...
public enum ColorRGB implements YggdrasilSerializable {
	...
	private ColorRGB(final DyeColor wool, final ChatColor chat, final org.bukkit.Color bukkit) {
		this.wool = wool;
		this.chat = chat;
		this.bukkit = bukkit;
	}
	
	private final static ColorRGB[] byWool = new ColorRGB[16];
	static {
		for (final ColorRGB c : values()) {
			byWool[c.wool.getWoolData()] = c;
		}
	}
	
	final static Map<String, ColorRGB> byName = new HashMap<>();
	final static Map<String, ColorRGB> byEnglishName = new HashMap<>();
	static {
	...
	@Nullable
	public static ColorRGB byName(final String name) {
		return byName.get(name.toLowerCase());
	}
	
	@Nullable
	public static ColorRGB byEnglishName(final String name) {
		return byEnglishName.get(name.toLowerCase());
	}
	
	@Nullable
	public static ColorRGB byWool(final short data) {
		if (data < 0 || data >= 16)
			return null;
		return byWool[data];
	}
	...
	@Nullable
	public static ColorRGB byDye(final short data) {
		if (data < 0 || data >= 16)
			return null;
		return byWool[15 - data];
	}
	
	public static ColorRGB byWoolColor(final DyeColor color) {
